...
inline void Boundary_Init(const int &numx,const int &numy,const double hx, std::vector<double>& u_temp);
...
int main(int argc, char *argv[])
{
  ...
  MPI_Comm_rank( MPI_COMM_WORLD, &rank );
    ...
    if(cart_rank==0)
    {
      ...
      Boundary_Init(numgridpoints_x,numgridpoints_y,hx,u_temp);
      ...
      double delta_r_1=0;
      for (size_t i = 0; i < ind.size(); i++) {
        double u_left= u_temp[ind[i]-1];
        double u_right= u_temp[ind[i]+1];
        double u_up= u_temp[ind[i]+numgridpoints_x];
        double u_down = u_temp[ind[i]-numgridpoints_x];
        double u_center= u_temp[ind[i]];
...
inline void Boundary_Init(const int &numx,const int &numy,const double hx, std::vector<double>& u_temp){
  ...
  for (int i = (numy-1) * numx; i < (numx * numy) ; ++i ) {
      u_temp[i] = (con_sinh* sin(2 * PI * (hx *(i - ((numy - 1) * numx)))));
